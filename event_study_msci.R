...
df_france_combined <- df_france_index %>%
  select(Date, return) %>%
  inner_join(df_msci %>% select(Date, return) %>% rename(bond_return = return), by = "Date")
...
for (i in 1:nrow(event_dates)) {
  ...
  model <- lm(return ~ bond_return, data = df_regression)
  
  # Alpha and Beta parameters
  alpha <- coef(model)[1]
  beta <- coef(model)[2]
  
  # Abnormal return (AR) calc.
  df_combined <- df_combined %>%
    mutate(
      predicted_return = alpha + beta * bond_return,
  ...
  s_n_squared <- ifelse(n_estimation > K,
                        sum((df_regression$return - (alpha + beta * df_regression$bond_return))^2, na.rm = TRUE) / (n_estimation - K),
                        NA)
  ...
  for (window_name in names(event_windows)) {
    ...
    # results
    results_msci <- rbind(results_msci, data.frame(
      country = country,
      event_date = day_0,
...
# Filename
file_path <- "event_study_results_msci.xlsx"

# Write dataframes to Excel
write_xlsx(list(
  #"Results_Final" = results_final,
  "Results" = results_msci
), path = file_path)
